...
var logger := CustomLogger.new(self)
...
class CustomLogger extends Logger:
	...
	func _init(console) -> void:
		self.console = console
	
	func _log_message(message: String, _error: bool) -> void:
		var formatted := message

		if message.begins_with("[DEBUG]"):
			formatted = "[color=#8888ff]%s[/color]" % message
		elif message.begins_with("[INFO]"):
			formatted = "[color=#cccccc]%s[/color]" % message

		self.console.call_deferred(&"append_text", formatted)
		#console.call_deferred(
			#&"append_text",
			#formatted + "\n"
		#)
